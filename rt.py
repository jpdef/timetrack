...
class RTHTMLPARSER(HTMLParser):
    ...
    def getData(self):
        self.extractedData = zip(
            self.ticketnumbers, self.ticketnames, self.queuenames)
...
class RT():
    ...
    def login(self):
        ...
        self.sess.post("https://rt.cosylab.com/rt3/login/login.cgi",
                       data=secrets.secrets, allow_redirects=True)

    def submitToTicket(self, datadir, msg, time, ticketnumber):
        self.login()
        data = {"UpdateContent": msg,
                "UpdateTimeWorked": time,
                "UpdateTimeWorked-Units": "minutes",
        ...
        self.sess.post(
            "https://rt.cosylab.com/rt3/Ticket/Update.html", data=data)
